...
class MainActivity : AppCompatActivity() {
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == 100 && resultCode == RESULT_OK) {
            ...
            screenLive.startLive(binding.url.text.toString().trim() + binding.code.text.toString().trim(), mediaProjection)
        }
    }

    fun startLive(view: View) {
        if (binding.url.text.toString().trim().isEmpty() || binding.code.text.toString().trim().isEmpty()) return
